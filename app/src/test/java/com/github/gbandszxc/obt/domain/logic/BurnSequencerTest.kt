...
/**
 * [BurnSequencer] 的纯逻辑测试：标准四阶段边界定位、阶段音源与打擂阶段 30 分钟轮换。
 * 边界值取自方案定义（阶段时长/音量沿原版逆向结论，音源为内置音乐移除后的本版编排）：
 * 舒筋 0..43199 白噪 / 活络 43200..86399 粉噪 / 习武 86400..345599 粉噪恒定 /
 * 打擂 345600..431999 白噪↔粉噪按「阶段内剩余秒/1800」奇偶轮换（偶→白噪、奇→粉噪）。
 */
class BurnSequencerTest {
    ...
    fun `第0秒位于舒筋阶段并使用白噪`() {
        ...
        assertEquals("舒筋", position.phase.name)
    ...
    // ---- 打擂阶段：按阶段内剩余秒每 30 分钟轮换（偶→白噪、奇→粉噪） ----

    @Test
    fun `打擂开场先播1秒白噪随后进入粉噪段`() {
