 ...
 *
 * 展示名不进本状态：只携带结构化身份（[planId] / [phaseIndex] / [soundSource] /
 ...
 * @property phaseIndex 当前所处阶段序号（多阶段方案随进度变化，与 [com.github.gbandszxc.obt.domain.model.BurnPhase.index] 同口径）。
 * @property soundSource 当前实际音源（含打擂轮换结果）；本地音乐阶段为
 *   [SoundSource.LOCAL_TRACK]；空闲/未知时为 null。
 * @property localTrackName 本地音乐音源阶段的曲目展示名（由 PlaybackController 切阶段时
 *   解析缓存）；非本地音乐阶段为 null。
 */
