...
// 产物命名规范：burn-in-tool-v<versionName>-<abi>-<variant>.apk
// （如 burn-in-tool-v1.5.0-arm64-v8a-release.apk）；abi 取 splits 生成的 ABI filter 标识，
// variant 即 release/debug；版本号引用 appVersionName 单一来源。
androidComponents {
    onVariants { variant ->
        variant.outputs.forEach { output ->
            val abi = output.filters.firstOrNull()?.identifier
            output.outputFileName.set("burn-in-tool-v$appVersionName-$abi-${variant.name}.apk")
        }
    }
}
