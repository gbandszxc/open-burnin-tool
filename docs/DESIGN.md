...
- **记录列表**：小结行（两列数据 + 行尾清除 IconButton）+ `HorizontalDivider(outlineVariant)` + LazyColumn（内容 padding 水平 24dp）；行内边距垂直 12dp；状态色进行中/已暂停用 `primary`、其余 `onSurfaceVariant`；会话行方案行（`bodySmall`/`onSurfaceVariant`）在自由煲机会话末尾追加「 · 音效名」（内置合成音源本地化名、本地音乐显示曲目名快照且缺省回退「本地音乐」；方案煲机与旧数据不追加），沿用该行既有字体与配色；播放器空闲时，进行中/已暂停、可重建方案的会话行（含进度为 0 的行：进度落库粒度 60 秒，起播未满一分钟即退出的会话进度仍为 0）在首行状态文本左侧显示播放三角 IconButton（`Icons.Filled.PlayArrow` 24dp、`primary` 着色、M3 默认 48dp 触达，contentDescription 走 `cd_history_resume_session`），点击续播该会话（`ui/history/HistoryTab.kt` `SummaryRow`/`SessionRow`）。
